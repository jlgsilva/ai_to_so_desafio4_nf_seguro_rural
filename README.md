...
- **Modelos padrão descontinuados pela Groq (16/08/2026)**: a Groq depreciou
  `llama-3.3-70b-versatile` e `llama-3.1-8b-instant`, os dois modelos usados
  por padrão nesta aplicação, quebrando as chamadas à API. A correção troca
  os defaults pelos substitutos recomendados pela própria Groq:
  `openai/gpt-oss-120b` (agente de raciocínio) e `openai/gpt-oss-20b`
  (tarefas rápidas). Os dois nomes de modelo agora vivem só em
  `src/llm_client.py`, num único bloco no topo do arquivo com instruções de
  como trocar de novo caso aconteça outra deprecação (ou via variáveis de
  ambiente `GROQ_MODEL_RACIOCINIO`/`GROQ_MODEL_RAPIDO`, sem editar código).
  Ver seção "Limites de tokens da conta Groq" para os novos números.
...
A tabela abaixo resume os limites relevantes da conta gratuita (ver valores
sempre atualizados em https://console.groq.com/settings/limits, pois a Groq
pode ajustar esses números sem aviso prévio):
...
> **Nota (17/08/2026)**: até 15/08/2026 a aplicação usava
> `llama-3.3-70b-versatile` (TPM 12.000, TPD 100.000) e
> `llama-3.1-8b-instant` (TPM 6.000, **RPD 14.400**). A Groq deprecou os
> dois em 16/08/2026 (ver changelog acima). Os substitutos oficiais têm TPD
> maior, mas RPD bem menor — em especial o modelo rápido, que caiu de
> 14.400 para 1.000 requisições/dia, o mesmo teto do modelo de raciocínio.

A aplicação usa, por padrão, `openai/gpt-oss-120b` para o agente de
raciocínio (Planner e Executor) e `openai/gpt-oss-20b` para as tarefas mais
simples (resumo do dataset, redação da resposta final) — ver
`src/llm_client.py`. A escolha do modelo de raciocínio prioriza o TPM, não
apenas o TPD: o perfil dos dados enviado ao Planner e ao Executor pode
chegar a alguns milhares de tokens em datasets com muitas colunas, e mais de
uma chamada costuma acontecer dentro do mesmo minuto (planejamento, geração
de SQL e, eventualmente, correção de erro). Um modelo com TPM baixo pode
rejeitar uma única chamada mesmo estando bem abaixo do limite diário, como
aconteceu em testes reais com `openai/gpt-oss-120b` (TPM 8.000): o perfil
compacto de três arquivos do SISSER com schema idêntico passava de 8.000
tokens numa única chamada. A aplicação corrige a causa raiz desse problema
agrupando arquivos com schema idêntico em uma única entrada do perfil (ver
"Correções de bugs relevantes"), o que reduz bastante o consumo por
pergunta, além de agora ter um cache de perguntas por sessão que evita
gastar tokens de novo em perguntas repetidas — o que ajuda a compensar o RPD
mais apertado do novo modelo rápido.

Se o limite ainda assim for atingido, os modelos podem ser trocados sem
mexer em variáveis de ambiente, editando só as duas linhas no topo de
`src/llm_client.py` (ou via variáveis de ambiente `GROQ_MODEL_RACIOCINIO` e
`GROQ_MODEL_RAPIDO`, ver `.env.example`, que têm prioridade sobre o código),
ou aguardando o horário de reset informado na mensagem de erro.
...
- **Groq** como provedor de LLM, modelos gratuitos (`openai/gpt-oss-120b`
  para planejamento e geração de SQL, `openai/gpt-oss-20b` para tarefas
  simples — nomes configuráveis em `src/llm_client.py`).
